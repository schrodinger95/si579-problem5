...
function addSaveButtons() {
    const saveButtons = wordOutput.getElementsByTagName('button');
...
function createButton() {
    ...
    saveButton.style.margin = "2px 5px";
...
function showRhymes() {
    ...
    wordOutput.innerHTML = '<div id="loading">...loading</div>';
    datamuseRequest(getDatamuseRhymeUrl(wordInput.value), (result) => {
        if (result.length > 0) {
            syllable = groupBy(result, 'numSyllables');
            Object.keys(syllable).forEach(function(key) {
                wordOutput.innerHTML += '<h3>Syllables: ' + key + '</h3>';
                const rhymeList = document.createElement('ul');
                syllable[key].forEach(element => {
                    const rhyme = document.createElement('li');
                    rhyme.textContent = element['word'];
                    rhyme.appendChild(createButton());
                    rhymeList.append(rhyme);
                });
                wordOutput.append(rhymeList);
            });
            addSaveButtons();
        }
        else {
    ...
    datamuseRequest(getDatamuseSimilarToUrl(wordInput.value), (result) => {
        if (result.length > 0) {
            ...
            result.forEach(element => {
                ...
                synonym.textContent = element['word'];
                synonym.appendChild(createButton());
                synonymList.append(synonym);
            });
            wordOutput.append(synonymList);
            addSaveButtons();
        }
        else {
            wordOutput.textContent = '(no results)';
